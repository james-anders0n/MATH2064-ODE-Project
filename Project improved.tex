...
\author{Riley Marriott, James Anderson, James Gregory, Andre Ristovsky}
...
Ordinary Differential Equations (ODEs) describe systems in which variables evolve over time according to specified rates of change \cite{2,13}. They are fundamental in many disciplines, including engineering, physics, and biology, where they are used to model dynamic processes such as fluid flow, motion, and population dynamics. In many practical situations, particularly when ODE systems are nonlinear and coupled, analytical solutions are unavailable, and numerical methods are required to approximate their behaviour \cite{1,3}.
A key application of such systems arises in epidemiology through compartmental models of infectious disease. The Susceptible--Infected--Recovered (SIR) model is a widely used framework that represents the spread of disease using a system of coupled nonlinear ODEs. It enables prediction of outbreak dynamics, including peak infection levels and long-term population behaviour, and is therefore an ideal case study for evaluating numerical methods.
This report investigates the performance of Euler, RK2, and RK4 methods for solving ODEs, using both a benchmark problem with a known analytical solution and the nonlinear SIR model. The methods are compared in terms of accuracy, stability, and computational efficiency, providing insight into their suitability for solving real-world dynamical systems.
...
\]
Figure~\ref{fig:comparison} shows that Euler deviates most from the exact solution, RK2 improves accuracy, and RK4 closely matches the analytical solution. This demonstrates how higher-order methods better capture the curvature of the solution, reducing the accumulation of error over time.

\begin{figure}[H]
\centering
\includegraphics[width=0.60\textwidth]{comparison_plot.png}
\caption{Numerical solutions of the benchmark ODE compared with the analytical solution over $[0,2]$, showing increasing accuracy from Euler to RK4.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.60\textwidth]{convergence_plot.png}
\caption{Log--log plot of maximum global error against step size, showing slopes corresponding to first-, second-, and fourth-order convergence for Euler, RK2, and RK4.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{timing_comparison.png}
\caption{Execution time of Euler, RK2, and RK4 methods for the benchmark problem, illustrating increased computational cost with additional function evaluations per step.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{error_vs_function_evaluations.png}
\caption{Maximum global error as a function of total function evaluations, demonstrating the improved efficiency of higher-order methods.}
...
All three methods were applied over the interval $[0,160]$ using a fixed step size of $h = 0.5$, allowing direct comparison of their performance on a nonlinear coupled system.

\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{sir_comparison.png}
\caption{Comparison of Euler, RK2, and RK4 methods for the SIR model, showing the infected population over time.}
\end{figure}

Figure 9 shows the evolution of the infected population computed using each numerical method. Euler’s method produces a solution that is close to the higher-order methods for the chosen step size, although small differences are visible near the peak infection. These differences arise from its lower accuracy and sensitivity to step size, which become more pronounced for larger time steps or more rapidly varying systems.

The RK2 method improves upon this behaviour, capturing the overall trend more accurately and reducing the error in peak prediction. RK2 provides results that closely align with RK4, improving accuracy relative to Euler while maintaining similar overall behaviour.

In contrast, the RK4 method produces a smooth and stable solution that closely represents the expected dynamics of the system. The infected population increases rapidly, reaches a peak of approximately $I_{\max} \approx 0.36$ at $t \approx 25$, and then declines as the recovered population increases. This behaviour is consistent with the basic reproduction number
\[
R_0 = \frac{\beta}{\gamma} = 3.5 > 1,
\]
indicating initial growth of the epidemic.

The simulation also shows that the SIR model is sensitive to its parameters and initial conditions. The transmission rate $\beta$ controls how quickly susceptible individuals become infected, while the recovery rate $\gamma$ controls how quickly infected individuals move into the recovered group. Increasing $\beta$ or decreasing $\gamma$ increases the basic reproduction number $R_0$, leading to a larger and faster outbreak. Similarly, changing the initial infected population affects how quickly the epidemic peak is reached.
The comparison demonstrates that higher-order methods are more reliable for nonlinear systems such as the SIR model, where small numerical errors can significantly affect the dynamics. While Euler’s method is computationally inexpensive, its reduced accuracy limits its usefulness for such problems. RK2 provides a reasonable compromise, but RK4 offers the best overall performance in terms of accuracy and stability.
...
The results demonstrate the trade-off between computational cost and accuracy, with RK4 generally providing the highest accuracy and stability.
...
Overall, RK4 provided the best balance between accuracy, stability, and computational efficiency. Although it requires more function evaluations per step, its higher-order accuracy allows larger step sizes to be used while maintaining low error, making it the most effective method for nonlinear problems of this type \cite{1}. These results highlight the importance of selecting numerical methods based on both problem characteristics and accuracy requirements.

\section{Extensions}

Further work could extend both the numerical methodology and the modelling framework considered in this report. One important direction is the use of adaptive step-size methods, where the time step $h$ is adjusted dynamically based on an estimate of the local truncation error. Adaptive Runge--Kutta schemes, such as embedded RK methods, can significantly improve computational efficiency by using larger step sizes in smooth regions and smaller step sizes where the solution varies rapidly \cite{16}.

Another extension involves the treatment of stiff systems, where explicit methods such as Euler and RK4 may become unstable unless extremely small step sizes are used. In such cases, implicit methods or semi-implicit schemes provide improved stability properties and are widely used in engineering and biological applications \cite{5,9}.

From a modelling perspective, the SIR model can be extended to more realistic epidemiological frameworks, such as the SEIR model, which introduces an exposed (latent) population, or age-structured and spatially distributed models. These extensions allow for more accurate representation of real-world disease dynamics, including incubation periods and heterogeneous mixing patterns \cite{14,15}.

Finally, further investigation could explore sensitivity analysis and parameter estimation, particularly for the transmission and recovery rates. This would provide deeper insight into how uncertainties in model parameters influence the predicted dynamics and could improve the reliability of numerical simulations in practical applications.
