...
This paper investigates the performance of several numerical methods for solving ordinary differential equations, with a particular focus on the Euler method, the second-order Runge--Kutta method (RK2), and the fourth-order Runge--Kutta method (RK4). The methods are first applied to a simple ODE with a known analytical solution to evaluate convergence behaviour and global error as a function of step size. They are then applied to the nonlinear Susceptible--Infected--Recovered (SIR) epidemic model to simulate the spread of disease within a population. The numerical solutions are evaluated in terms of accuracy, stability, and computational efficiency, providing insight into the relative performance of each method when modelling complex real-world systems. The results demonstrate the trade-off between numerical accuracy and computational cost, illustrating the advantages of higher-order Runge--Kutta methods for modelling nonlinear dynamical systems.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.70\textwidth]{sir_rk4.png}
\caption{Time evolution of susceptible, infected, and recovered populations computed using RK4, showing epidemic growth, peak infection, and recovery dynamics.}
\label{fig:sir-rk4}
\end{figure}
